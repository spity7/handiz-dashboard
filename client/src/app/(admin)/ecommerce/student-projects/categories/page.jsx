...
const StudentProjectCategoriesPage = () => {
  const { getStudentProjectCategories, createStudentProjectCategory, updateStudentProjectCategory, deleteStudentProjectCategory } = useGlobalContext()
  ...
  const confirmField = useConfirmFieldForm('project category')

  const load = useCallback(async () => {
    try {
      const list = await getStudentProjectCategories()
  ...
  const handleCreate = async (e) => {
    ...
    await confirmField('create', async () => {
      try {
        setSavingNew(true)
        await createStudentProjectCategory(name)
        setNewName('')
        await load()
  ...
  const saveEdit = async (id) => {
    ...
    await confirmField('update', async () => {
      try {
        setSavingEdit(true)
        await updateStudentProjectCategory(id, name)
        cancelEdit()
        await load()
  ...
  const handleDelete = async (c) => {
    ...
    try {
      const data = await deleteStudentProjectCategory(c._id)
      await load()
      const n = data?.reassignedCount ?? 0
      Swal.fire('Deleted', n > 0 ? `${n} project(s) updated with Others.` : 'Category removed.', 'success')
    } catch (error) {
  ...
  return (
    <>
      <PageMetaData title="Student Project Categories" />
      <PageBreadcrumb title="Student Project Categories" subName="Handiz" />
      <Row>
        <Col>
          <Card>
            <CardBody>
...
export default StudentProjectCategoriesPage
