...
const GeneralDetailsForm = () => {
  ...
  const { isDirty: hasChanges } = useRegisterRhfFormDirty(STUDENT_PROJECT_FORM_DEFAULTS, formValues, {
  ...
  const onSubmit = async (data) => {
    ...
    const submitProject = async () => {
      try {
        ...
        await withAsyncToast(() => createProject(formData), {
          loading: 'Creating project…',
          success: 'Project created successfully.',
          error: (error) => error?.response?.data?.message || 'Failed to create project',
        })
  ...
  return (
    <fieldset disabled={loading} style={{ border: 'none', margin: 0, padding: 0 }}>
      <form noValidate onSubmit={handleSubmit(onSubmit, onInvalid)}>
        ...
        <Button type="submit" disabled={loading || !hasChanges} className="mt-4">
